...
#     # Replace placeholders with stored context values
#     request_data = row['request']
#
#     if isinstance(request_data, str):
#         for key in test_executor.store:
#             request_data = request_data.replace(f"{{{key}}}", str(test_executor.get(key)))
#
#     try:
#         request_json = json.loads(request_data)  # Ensure request data is valid JSON
#     except json.JSONDecodeError as e:
#         pytest.fail(f"Invalid JSON format after replacement: {request_data} - Error: {e}")
#
#     # Make API request
...
def test_api_cases(test_executor, index, row):
    ...
    if request_json is None:
        pytest.fail("request_json is None after JSON parsing failure!")
    # Make API request
    url = BASE_URL + row['endpoint']
    headers = {"Content-Type": "application/json", "Authorization": f"Bearer {BEARER_TOKEN}"}
    response = requests.request(row['request_type'], url, json=request_json, headers=headers)
