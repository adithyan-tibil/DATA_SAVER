...
BEGIN
	...
	-------------------------------BIND_DEVICE---------------------------------------------------------------
        WHEN evt='BIND_DEVICE' THEN
            ...
            IF EXISTS (SELECT 1 FROM registry.sb WHERE did = d_id AND vid IS NOT NULL AND isd = FALSE AND isa = True) THEN
                messages := array_append(messages, 'DEVICE_ALREADY_BINDED'::registry.sb_msgs);
            END IF;

			-- SELECT bid INTO values_tester FROM registry.sb WHERE did = d_id AND isd = FALSE AND isa = True;
			-- IF values_tester IS NOT NULL THEN
			-- 	IF NOT EXISTS (SELECT 1 FROM registry.banks WHERE bid=values_tester and isd = FALSE AND isa = True) THEN
        ...
        WHEN evt='UNBIND_DEVICE' THEN
            IF NOT EXISTS (SELECT 1 FROM registry.vpa WHERE vid = v_id AND isd = FALSE AND isa = True) THEN
            ...
            IF NOT EXISTS (SELECT 1 FROM registry.sb WHERE did = d_id AND vid = v_id AND isd = FALSE AND isa = True) THEN
        ...
        WHEN evt='ALLOCATE_TO_BANK' THEN
            ...
            IF EXISTS (SELECT 1 FROM registry.sb WHERE did = d_id AND bid IS NOT NULL AND isd = FALSE AND isa = True) THEN
                messages := array_append(messages, 'BANK_ALREADY_ALLOCATED'::registry.sb_msgs);
            END IF;			

			-- SELECT vid INTO values_tester FROM registry.sb WHERE did = d_id AND isd = FALSE AND isa = True;
			-- IF values_tester IS NOT NULL THEN
			-- 	IF NOT EXISTS (SELECT 1 FROM registry.vpa WHERE vid=values_tester and isd = FALSE AND isa = True) THEN
			-- 		messages := array_append(messages, 'DEVICE_HAS_DEACTIVATED_VPA'::registry.sb_msgs);
			-- 	END IF;	
			-- END IF;			

			SELECT bid INTO values_tester FROM registry.vpa WHERE vid = (
			SELECT vid FROM registry.sb WHERE did = d_id AND isd = FALSE AND isa = True) AND isd = FALSE AND isa = True;
			IF values_tester IS NOT NULL THEN
				IF values_tester != b_id THEN
					messages := array_append(messages, 'BANK_VPA_UNMATCHED_BID'::registry.sb_msgs);
				END IF;
			END IF;
        ...
        WHEN evt='ALLOCATE_TO_BRANCH' THEN
            ...
            IF EXISTS (SELECT 1 FROM registry.sb WHERE did = d_id AND brid IS NOT NULL AND isa = True) THEN
			...
			-- END IF;	
   			...
   			SELECT bid INTO values_tester FROM registry.sb WHERE did = d_id AND isd = FALSE AND isa = True;
    		IF values_tester IS NOT NULL THEN
				SELECT bid INTO values_tester2 FROM registry.branches WHERE brid = br_id AND isd = FALSE AND isa = True;
        		IF values_tester != values_tester2 THEN
            		messages := array_append(messages, 'DEVICE_BRANCH_UNMATCHED_BID'::registry.sb_msgs);
        		END IF;
    		END IF;
        ...
        WHEN evt='ALLOCATE_TO_MERCHANT' THEN
			...
			IF EXISTS ( SELECT 1 FROM registry.sb WHERE did = d_id AND isa = True AND isd = FALSE AND (bid IS NULL OR brid IS NULL OR vid IS NULL) ) THEN
            ...
            IF EXISTS (SELECT 1 FROM registry.sb WHERE did = d_id AND mid IS NOT NULL AND isd = FALSE AND isa = True) THEN
                messages := array_append(messages, 'MERCHANT_ALREADY_ALLOCATED'::registry.sb_msgs);
            END IF;

			-- SELECT brid INTO values_tester FROM registry.sb WHERE did = d_id AND isd = FALSE AND isa = True;
			-- IF values_tester IS NOT NULL THEN
			-- 	IF NOT EXISTS (SELECT 1 FROM registry.branches WHERE brid=values_tester and isd = FALSE AND isa = True) THEN
			...
			-- END IF;
   			...
   			SELECT brid INTO values_tester FROM registry.sb WHERE did = d_id AND isd = FALSE AND isa = True;
    		SELECT brid INTO values_tester2 FROM registry.merchants WHERE mpid = mp_id AND isd = FALSE AND isa = True;
    		IF values_tester IS NOT NULL AND values_tester2 IS NOT NULL THEN
        		IF values_tester != values_tester2 THEN
            		messages := array_append(messages, 'DEVICE_MERCHANT_UNMATCHED_BRID'::registry.sb_msgs);
        		END IF;
    		END IF;
        ...
        WHEN evt='REALLOCATE_TO_MERCHANT' THEN
            ...
            IF EXISTS (SELECT 1 FROM registry.sb WHERE mid=mp_id AND isd = FALSE AND isa = True) THEN
                messages := array_append(messages, 'MERCHANT_ALREADY_ALLOCATED'::registry.sb_msgs);
            END IF;
        ...
        WHEN evt='REALLOCATE_TO_BANK' THEN
            IF NOT EXISTS (SELECT 1 FROM registry.merchants WHERE bid = b_id AND isd = FALSE AND isa = True) THEN
                messages := array_append(messages, 'INVALID_BANK'::registry.sb_msgs);
            END IF;
