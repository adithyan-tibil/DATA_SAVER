...
BEGIN
	...
	-------------------------------BIND_DEVICE---------------------------------------------------------------
        WHEN evt='BIND_DEVICE' THEN
            ...
            IF NOT EXISTS (SELECT 1 FROM registry.sb WHERE did = d_id AND isd = FALSE AND isa = TRUE) THEN
        ...
        WHEN evt='ALLOCATE_TO_BANK' THEN
            ...
            IF EXISTS (SELECT bid FROM registry.sb WHERE did = d_id  AND isd = FALSE AND isa = TRUE) THEN
        ...
        WHEN evt='ALLOCATE_TO_BRANCH' THEN
            ...
            IF EXISTS (SELECT brid FROM registry.sb WHERE did = d_id AND isd = FALSE AND isa = TRUE) THEN
