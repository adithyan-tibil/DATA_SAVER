	...
	-------------------------------BIND_DEVICE---------------------------------------------------------------
        WHEN evt='BIND_DEVICE' THEN
            ...
            IF EXISTS (SELECT 1 FROM registry.sb WHERE did = d_id AND vid IS NOT NULL AND isd = FALSE and isa = TRUE) THEN
                messages := array_append(messages, 'VPA_ALREADY_BINDED'::registry.sb_msgs);
            END IF;

			IF EXISTS (SELECT 1 FROM registry.sb WHERE vid = v_id AND isa = TRUE) THEN
                messages := array_append(messages, 'VPA_REPEATED'::registry.sb_msgs);
            END IF;		

			IF NOT EXISTS(SELECT 1 FROM registry.vpa WHERE vid = v_id AND isa = TRUE) THEN
        ...
        WHEN evt='ALLOCATE_TO_BRANCH' THEN
            ...
            IF EXISTS (SELECT 1 FROM registry.sb WHERE did = d_id AND brid IS NOT NULL AND isd = FALSE AND isa = TRUE) THEN
