ALTER TABLE registry.devices ADD COLUMN ln INTEGER
...
BEGIN
    validator_result := registry.update_device_validator(f_name,sb_e,lang);
...
BEGIN
    ...
    IF f_name IS NOT NULL AND NOT EXISTS (
        SELECT 1 FROM registry.firmware WHERE fname = f_name
    ) THEN
...
--     1,                 -- rowid
--     10,                -- d_id (device ID)
--     'FIRMWARE',     -- f_name (firmware name, nullable)
--     NULL::INTEGER,                 -- lang (language ID as integer)
