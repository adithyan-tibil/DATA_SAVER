...
-- updates the ourtput contains multiple id
...
-- RETURNS TABLE (row_id INTEGER, status INTEGER, msg TEXT[],sb_id INTEGER,vpa_id INTEGER,device_id INTEGER,bank_id INTEGER,branhc_id INTEGER,merchant_id INTEGER,e_at timestamp) AS $$
RETURNS TABLE (row_id INTEGER, status INTEGER, msg TEXT[],id_headers TEXT[],id_values INT[],e_at timestamp) AS $$
DECLARE
...
BEGIN
	...
	IF array_length(validator_result, 1) >0 THEN
		RETURN QUERY SELECT rowid,0,validator_result,ARRAY[NULL],ARRAY[NULL],e_at; 
		...
		CASE
			WHEN evt='VPA_DEVICE_BOUND' THEN
				...
				RETURNING sid,vid,did,bid,brid,mpid,eat INTO id_values;
       			RETURN QUERY SELECT rowid, 1, ARRAY['VPA_DEVICE_BOUNDED'], headers_array,ARRAY[id_values.sid, id_values.vid, id_values.did, id_values.bid, id_values.brid, id_values.mpid],e_at;
			...
			WHEN evt='ALLOCATE_TO_BANK' THEN
				UPDATE registry.sb
       			...
       			WHERE did = d_id AND isd = FALSE
				RETURNING sid,vid,did,bid,brid,mpid,eat INTO id_values;
       			RETURN QUERY SELECT rowid, 1, ARRAY['ALLOCATED_TO_BANK'],  headers_array,ARRAY[id_values.sid, id_values.vid, id_values.did, id_values.bid, id_values.brid, id_values.mpid],e_at;
			...
			WHEN evt='ALLOCATE_TO_BRANCH' THEN
				UPDATE registry.sb
       			...
       			WHERE did = d_id AND isd = FALSE
				RETURNING sid,vid,did,bid,brid,mpid,eat INTO id_values;
       			RETURN QUERY SELECT rowid, 1, ARRAY['ALLOCATED_TO_BRANCH'],  headers_array,ARRAY[id_values.sid, id_values.vid, id_values.did, id_values.bid, id_values.brid, id_values.mpid],e_at;
			...
			WHEN evt='ALLOCATE_TO_MERCHANT' THEN
				UPDATE registry.sb
       			...
       			WHERE did = d_id AND isd = FALSE
				RETURNING sid,vid,did,bid,brid,mpid,eat INTO id_values;
       			RETURN QUERY SELECT rowid, 1, ARRAY['ALLOCATED_TO_MERCHANT'], headers_array,ARRAY[id_values.sid, id_values.vid, id_values.did, id_values.bid, id_values.brid, id_values.mpid],e_at;
