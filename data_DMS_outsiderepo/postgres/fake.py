    ...
    with conn.cursor() as cur:
        for _ in range(n):
            data = (
                ...
                random.choice(['ONBOARDED', 'DEACTIVATED']),
                fake.random_int(min=100, max=99999),
                fake.random_int(min=100, max=99999),
                False
            )
...
def insert_firmware(mf_ids, n=10):
    ...
    with conn.cursor() as cur:
        for _ in range(n):
            ...
            cur.execute("""
                INSERT INTO dmsr.firmware (mfid, fdesc, eid, eby, isd) 
                VALUES (%s, %s, %s, %s, %s) RETURNING fid;
            """, data)
...
def insert_model(mf_ids, firmware_ids, n=10):
    ...
    with conn.cursor() as cur:
        for _ in range(n):
            ...
            cur.execute("""
                INSERT INTO dmsr.model (mfid, fid, eid, mdesc, isd, eby) 
                VALUES (%s, %s, %s, %s, %s, %s) RETURNING mdid;
            """, data)
...
def insert_devices(mf_ids, firmware_ids, model_ids, n=10):
    ...
    with conn.cursor() as cur:
        for _ in range(n):
            ...
            cur.execute("""
                INSERT INTO dmsr.devices (dname, mfid, fid, mdid, eid, isd, eby) 
                VALUES (%s, %s, %s, %s, %s, %s, %s) RETURNING did;
            """, data)
...
def insert_branches(bank_ids, n=10):
    ...
    with conn.cursor() as cur:
        for _ in range(n):
            ...
            cur.execute("""
                INSERT INTO dmsr.branches (brname, braddr, bid, eid, isd, eby) 
                VALUES (%s, %s, %s, %s, %s, %s) RETURNING brid;
            """, data)
...
def insert_sb(vpa_ids, merchant_ids, device_ids, bank_ids, branch_ids, n=10):
    with conn.cursor() as cur:
        for _ in range(n):
            ...
            cur.execute("""
                INSERT INTO dmsr.sb (vid, mid, did, bid, brid, eid, isd, eby) 
