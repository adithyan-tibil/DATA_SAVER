...
CREATE OR REPLACE FUNCTION registry.mf_validator(mf_name VARCHAR,mf_addr VARCHAR,mf_info JSONB) 
...
BEGIN
	...
	validator_result :=  registry.mf_validator(mf_name,mf_addr,mf_info);
    IF array_length(validator_result, 1) > 0 THEN
...
SELECT aid, ainfo, ecode, htype  from workflow.wfactions where ecode = 'UPDATE_BANK' and isd = FALSE