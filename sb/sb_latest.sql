...
BEGIN
	...
	-------------------------------BIND_DEVICE---------------------------------------------------------------
        WHEN evt='BIND_DEVICE' THEN
            ...
            IF EXISTS (SELECT vid FROM registry.sb WHERE did = d_id AND isd = FALSE) THEN
                messages := array_append(messages, 'DEVICE_ALREADY_BINDED'::registry.sb_msgs);
            END IF;

			
			IF NOT EXISTS (SELECT 1 FROM registry.banks WHERE bid=(SELECT bid FROM registry.sb WHERE did = d_id AND isd = FALSE ) and isd = FALSE AND isa = TRUE) THEN
        ...
        WHEN evt='ALLOCATE_TO_BANK' THEN
            ...
            IF EXISTS (SELECT bid FROM registry.sb WHERE did = d_id  AND isd = FALSE) THEN
        ...
        WHEN evt='ALLOCATE_TO_BRANCH' THEN
            ...
            IF EXISTS (SELECT brid FROM registry.sb WHERE did = d_id ) THEN
        ...
        WHEN evt='ALLOCATE_TO_MERCHANT' THEN
            ...
            IF NOT EXISTS (SELECT 1 FROM registry.devices WHERE did = d_id AND isd = FALSE AND isa = TRUE AND isa = TRUE) THEN
                messages := array_append(messages, 'INVALID_DEVICE'::registry.sb_msgs);
            END IF;

            IF EXISTS (SELECT mid FROM registry.sb WHERE did = d_id AND isd = FALSE) THEN
                messages := array_append(messages, 'MERCHANT_ALREADY_ALLOCATED'::registry.sb_msgs);
            END IF;

			
			IF NOT EXISTS (SELECT 1 FROM registry.branches WHERE brid=(SELECT brid FROM registry.sb WHERE did = d_id AND isd = FALSE) and isd = FALSE AND isa = TRUE) THEN
				messages := array_append(messages, 'DEVICE_DEACTIVATED_BRANCH'::registry.sb_msgs);
			END IF;	
			...
			IF NOT EXISTS (SELECT 1 FROM registry.vpa WHERE vid=(SELECT vid FROM registry.sb WHERE did = d_id AND isd = FALSE;) and isd = FALSE AND isa = TRUE) THEN
				messages := array_append(messages, 'DEVICE_DEACTIVATED_VPA'::registry.sb_msgs);
			END IF;
